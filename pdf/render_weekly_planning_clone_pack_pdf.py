...
PACK_PATH = ROOT.parent / "products" / "weekly-planning-clone-pack.md"
OUTPUT_PATH = ROOT / "weekly-planning-clone-pack.pdf"
COVER_ART = ROOT / "assets" / "weekly-planning-clone-pack-cover.png"
...
def draw_body(canvas, doc):
    ...
    canvas.drawString(doc.leftMargin, height - 34, "WEEKLY PLANNING CLONE PACK")
...
def build_cover(styles: StyleSheet1):
    ...
    story.append(
        Paragraph(
            "A practical system for using a personal AI clone to run a calmer weekly reset, sort open loops, choose realistic priorities, and prepare the week ahead.",
            ParagraphStyle(
                "CoverSub",
                parent=styles["Lead"],
                alignment=TA_CENTER,
    ...
    story.append(
        Paragraph(
            "A strong weekly planning clone helps you see the shape of the week before it fills up, so priorities stay realistic and loose ends stop spreading quietly.",
            ParagraphStyle(
                "CoverNote",
                parent=styles["Body"],
                alignment=TA_CENTER,
...
def build_quick_start(styles: StyleSheet1):
    rows = [
        ["Look back", "Review what was completed, what stayed open, and what created friction last week."],
        ["Clear backlog", "Sort carry-forwards, loose notes, and unfinished follow-ups into clear buckets."],
        ["Set priorities", "Choose realistic priorities based on deadlines, constraints, and attention limits."],
        ["Review rule", "Use the clone to prepare the week, not to promise more than the week can hold."],
    ]
    table = Table([["WEEKLY RHYTHM", "WHAT THE CLONE HELPS WITH"], *rows], colWidths=[1.45 * inch, 4.85 * inch], repeatRows=1)
    table.setStyle(
        TableStyle(
            [
    ...
    story = [
        ...
        Paragraph(
            "This pack works best when the clone supports a simple weekly rhythm. Start with one short weekly review, then add backlog triage and focus protection only if they genuinely reduce hidden overload.",
            styles["Lead"],
        ),
        table,
        Spacer(1, 16),
