...
PACK_PATH = ROOT.parent / "products" / "prompt-starter-pack.md"
OUTPUT_PATH = ROOT / "prompt-starter-pack.pdf"
COVER_ART = ROOT / "assets" / "prompt-starter-pack-cover.png"
...
def draw_body(canvas, doc):
    ...
    canvas.drawString(doc.leftMargin, height - 34, "PROMPT STARTER PACK")
...
def parse_markdown(markdown_text: str, styles: StyleSheet1):
    ...
    def flush_bullets():
        ...
        for bullet in bullet_lines:
            items.append(ListItem(Paragraph(escape(bullet), styles["BulletBody"]), leftIndent=0, value="circle"))
        story.append(
            ListFlowable(
                items,
                bulletType="bullet",
...
def build_cover(styles: StyleSheet1):
    ...
    story.append(
        Paragraph(
            "A clear, practical guide to writing better prompts for your personal AI clone, with stronger roles, tone, structure, and boundaries.",
            ParagraphStyle(
                "CoverSub",
                parent=styles["Lead"],
                alignment=TA_CENTER,
    ...
    story.append(
        Paragraph(
            "Better prompts do not try to sound clever. They try to make the work easier.",
            ParagraphStyle(
                "CoverNote",
                parent=styles["Body"],
                alignment=TA_CENTER,
...
def build_quick_reference(styles: StyleSheet1):
    rows = [
        ["Role", "Who the clone should be in this moment."],
        ["Task", "What exactly it should do."],
        ["Context", "What it should know before answering."],
        ["Format", "How the result should be structured."],
        ["Boundaries", "What it should not do or assume."],
    ]
    table = Table([["PROMPT PART", "WHY IT MATTERS"], *rows], colWidths=[1.4 * inch, 4.9 * inch], repeatRows=1)
    table.setStyle(
        TableStyle(
            [
    ...
    chips = Table(
        [
            ["Useful verbs", "organize, summarize, clarify, compare, draft, prioritize, refine"],
            ["Useful tones", "warm, direct, calm, practical, concise, supportive"],
            ["Useful formats", "bullets, numbered steps, short email, table, today / this week / later"],
        ],
    ...
    chips.setStyle(
        TableStyle(
            [
                ...
                ("ROWBACKGROUNDS", (0, 0), (-1, -1), [colors.white, colors.HexColor("#faf5ed")]),
                ("BOX", (0, 0), (-1, -1), 1, PALETTE["line"]),
                ("INNERGRID", (0, 0), (-1, -1), 0.6, PALETTE["line"]),
    ...
    story = [
        Paragraph("Quick Reference", styles["H2"]),
        Paragraph(
            "Use this page when you want a fast reminder of what makes a prompt more useful. The goal is not to sound technical. The goal is to give the clone enough structure to return something clear and usable.",
            styles["Lead"],
        ),
        table,
        Spacer(1, 16),
        ...
        Spacer(1, 16),
        Paragraph(
            "A strong prompt usually becomes clearer when you define the role, tighten the task, add the missing context, specify the output, and state what the clone should not do.",
            styles["Callout"],
        ),
        PageBreak(),
    ]
...
def main():
    ...
    story.extend(build_quick_reference(styles))
