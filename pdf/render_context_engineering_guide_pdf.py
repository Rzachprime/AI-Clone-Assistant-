...
PACK_PATH = ROOT.parent / "products" / "context-engineering-guide.md"
OUTPUT_PATH = ROOT / "context-engineering-guide.pdf"
COVER_ART = ROOT / "assets" / "context-engineering-guide-cover.png"
...
def draw_body(canvas, doc):
    ...
    canvas.drawString(doc.leftMargin, height - 34, "CONTEXT ENGINEERING GUIDE")
...
def build_cover(styles: StyleSheet1):
    ...
    story.append(
        Paragraph(
            "A practical guide to capturing, structuring, and reusing the context that makes a personal AI clone more relevant, more consistent, and more proactive over time.",
            ParagraphStyle(
                "CoverSub",
                parent=styles["Lead"],
                alignment=TA_CENTER,
    ...
    story.append(
        Paragraph(
            "The real breakthrough is not just better prompts. It is giving the clone the context it needs to support repeated work without starting from zero each time.",
            ParagraphStyle(
                "CoverNote",
                parent=styles["Body"],
                alignment=TA_CENTER,
...
def build_quick_start(styles: StyleSheet1):
    rows = [
        ["Start passive", "Begin with a low-risk capture workflow like meeting transcripts, summaries, and follow-ups."],
        ["Structure signal", "Keep the context relevant, current enough, and organized around the task that needs support."],
        ["Reuse context", "Turn repeated notes, decisions, and follow-ups into a context stack the clone can work from again."],
        ["Review rule", "Use Context Engineering to improve continuity first. Let trust grow before autonomy does."],
    ]
    table = Table([["CONTEXT STACK", "WHAT THE CLONE HELPS WITH"], *rows], colWidths=[1.45 * inch, 4.85 * inch], repeatRows=1)
    table.setStyle(
        TableStyle(
            [
    ...
    story = [
        ...
        Paragraph(
            "This guide works best when the clone already has a useful prompt or recurring task to support. Start by improving continuity and signal before you try to make the system more autonomous.",
            styles["Lead"],
        ),
        table,
        Spacer(1, 16),
        Paragraph(
            "Do not feed the clone everything. Give it the right context for one recurring use case first, then expand only if the results become more useful and more trustworthy.",
            styles["Callout"],
        ),
        PageBreak(),
    ]
