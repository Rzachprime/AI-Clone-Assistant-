...
PACK_PATH = ROOT.parent / "products" / "agentic-workflow-foundations.md"
OUTPUT_PATH = ROOT / "agentic-workflow-foundations.pdf"
COVER_ART = ROOT / "assets" / "agentic-workflow-foundations-cover.png"
...
def draw_body(canvas, doc):
    ...
    canvas.drawString(doc.leftMargin, height - 34, "AGENTIC WORKFLOW FOUNDATIONS")
...
def build_cover(styles: StyleSheet1):
    ...
    story.append(
        Paragraph(
            "A grounded guide to turning useful prompts into trustworthy workflows with triggers, boundaries, review points, and progressive autonomy.",
            ParagraphStyle(
                "CoverSub",
                parent=styles["Lead"],
                alignment=TA_CENTER,
    ...
    story.append(
        Paragraph(
            "The goal is not full autonomy. It is building calm, bounded workflows that carry low-risk work forward while the user keeps judgment and control.",
            ParagraphStyle(
                "CoverNote",
                parent=styles["Body"],
                alignment=TA_CENTER,
...
def build_quick_start(styles: StyleSheet1):
    rows = [
        ["Start small", "Move from one useful prompt to one repeatable workflow before you add automation."],
        ["Design the lane", "Define the trigger, inputs, processing steps, output, review point, and stop condition."],
        ["Bound the work", "Keep the workflow inside low-risk tasks with clear rules about what it must never do."],
        ["Review rule", "Trust should increase before autonomy does. Oversight stays in place as the workflow grows."],
    ]
    table = Table([["WORKFLOW MODEL", "WHAT THE CLONE HELPS WITH"], *rows], colWidths=[1.45 * inch, 4.85 * inch], repeatRows=1)
    table.setStyle(
        TableStyle(
            [
    ...
    story = [
        ...
        Paragraph(
            "This guide works best when the clone already feels useful in a small, repeatable way. Start with one bounded workflow, then add triggers and forward motion only after the review process feels trustworthy.",
            styles["Lead"],
        ),
        table,
        Spacer(1, 16),
